...
import {
    ...
    type WorldSeedCatalogIdLocation,
} from './contentCatalogWorldSeed';

type CatalogIdRegistryName = 'card' | 'deck' | 'world item';

type CatalogIdLocation = WorldSeedCatalogIdLocation;
...
function buildContentIdRegistries(
    ...
    const invalidCanonicalWorldSeedResourceIds = validateCanonicalWorldSeedCatalogEntries(resourceList, failures);
    const registries: ContentIdRegistries = {
        ...
        canonicalConfigs,
        worldItems: new Map(),
        invalidCanonicalWorldSeedResourceIds,
    };
    ...
    for (const resource of resourceList) {
        ...
        if (resource.entry.kind === 'worldSeed') {
            registerWorldItemIds(resource, registries.worldItems, failures);
        }
    }
...
function validateContentIdReferences(
    ...
    for (const resource of resources) {
        ...
        if (resource.entry.kind === 'worldSeed') {
            validateCanonicalWorldSeedReferences(resource, {
                decks: registries.decks,
                worldItems: registries.worldItems,
            }, failures);
        }
    }
}

function validateRegisteredIdReference<TLocation extends CatalogIdLocation>(
