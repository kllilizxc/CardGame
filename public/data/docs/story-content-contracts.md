...
| Kind | Required fields | Meaning |
| --- | --- | --- |
| `hub` | `hubId`, `hubFile` | Start `HubScene` with the declared Hub identity and Hub JSON file. Checked-in Hub destinations are `destination.qingyun-town` for `hub.qingyun-town` / `data/hub/town-shell.json` and `destination.qingyun-sect-gate` for `hub.qingyun-sect-gate` / `data/hub/qingyun-sect-gate.json`. |
| `expedition` | `expeditionId`, `mapId`, `worldStateFile`, `starterDeckFile`, `mapFile`, `eventsFile`, `shopFile` | Start `ExpeditionScene` with the declared Expedition identity and content files. The first checked-in destination is `destination.qingyun-outer-mountain-trial` for `phase01-first-playable-expedition` / `phase01-prototype-map` / `data/mijing/prototype-map.json`. |

`WorldMapScene` only owns this routing shell. It does not own unlock state, procedural overworld movement, Hub shops/training/inventory, Expedition rewards, Expedition active-run persistence, or migration of `mijing` events into Story JSON. Add a new destination id for a new long-lived route instead of repurposing an existing id. `HubScene` and `ExpeditionScene` both normalize their launch payloads with safe defaults so direct scene starts still load the checked-in Qingyun town / prototype expedition files, but world-map launches must pass the destination-owned target fields. `ExpeditionScene` carries that normalized target config into `BattleScene` launch payloads and back through expedition battle results so battle round-trips resume against the same world-map-owned route config.
...
4. Run `bun test src/game/scenes/worldmap/worldMap.test.ts src/game/scenes/hub/hubSceneLaunch.test.ts src/game/scenes/expedition/expeditionSceneLaunch.test.ts src/game/scenes/startupFlow.test.ts` to validate the world-map data contract, launch-payload defaults, and `MainMenu -> WorldMapScene -> HubScene | ExpeditionScene -> WorldMapScene` routing/resume loop.
