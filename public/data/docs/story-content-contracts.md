...
`src/game/content/contentCatalog.ts` performs pure catalog-backed target resolution for WorldMap-owned Hub and Expedition target resources, Hub-owned `startStory.storyResourceId` story targets, and Story-owned `startBattle.encounterResourceId` / `deckResourceId` battle targets. Catalog validation fails actionably when a destination, Hub story action, or Story battle trigger references a missing resource id, a resource id with the wrong kind, or a resource id whose catalog `publicPath` no longer matches the runtime file path (`hubFile`, `storyGraphFile`, `encounterFile`, `deckFile`, or Expedition target files). This keeps resource ids stable for tooling and review while preserving the existing scene loaders.
