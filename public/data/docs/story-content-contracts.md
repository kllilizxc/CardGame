...
| Kind | Required fields | Meaning |
| --- | --- | --- |
| `hub` | `hubId`, `hubFile` | Start `HubScene` with the declared Hub identity and Hub JSON file. The first checked-in destination is `destination.qingyun-town` for `hub.qingyun-town` / `data/hub/town-shell.json`. |
