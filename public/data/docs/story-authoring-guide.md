...
- **青云镇 Hub 入口文件**：`public/data/hub/town-shell.json`（顶层 Hub 子地图 `presentation` 与每地点 marker presentation；`navigate.targetLocationId` 在城镇小地点间切换；`startStory.storyResourceId` 指向 catalog story 资源，`startStory.storyGraphFile` 是 StoryScene 实际加载的剧情图路径）
