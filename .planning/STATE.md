...
- `GameWorldStatePersistentStashWrite` is the narrow write-facing seam over the current `cardgame.persistent-stash.v1` compatibility key. It builds from `GameWorldState`, requires an explicit `Storage`-like adapter, validates the persistent-stash compatibility metadata, deep-clones stored-stash or seed-fallback documents, and delegates the actual write to `savePersistentStash` without touching ambient `globalThis.localStorage`, Story/Hub session, active-run routing, Expedition scene flow, RunResolution math, UI, or content data.
