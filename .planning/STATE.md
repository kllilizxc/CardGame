...
- `GameWorldStateStoryHubSessionWrite` is the narrow plan/apply seam over the current `cardgame.story-hub-session.v1` compatibility key. It validates Story/Hub compatibility metadata, schema version 1, and Hub/Story session identity keys, deep-clones from-view/from-document plans and results, and delegates document writes through `StoryHubSessionPersistence` using an explicit `Storage`-like adapter; HubScene/StoryScene default fallback behavior, save key, and scene-triggered save/restore flow remain unchanged.
