...
- `RunPersistence` still owns persistent-stash writes, but `savePersistentStash` now accepts the same explicit `Storage`-like adapter seam as the read path. `ExpeditionState.bootstrap` threads that adapter for seed-fallback stash persistence and active-run writes; `RunResolution` threads it for terminal stash writes and active-run cleanup, preserving default localStorage/memory production behavior without making `GameWorldState` a writer.
- `GameWorldStatePersistentStashWrite` is the narrow write-facing seam over the current `cardgame.persistent-stash.v1` compatibility key. It builds from `GameWorldState`, requires an explicit `Storage`-like adapter, validates the persistent-stash compatibility metadata, deep-clones stored-stash or seed-fallback documents into a write plan, and delegates the apply step to `savePersistentStash` without touching ambient `globalThis.localStorage`, Story/Hub session, active-run routing, Expedition scene flow, RunResolution math, UI, or content data.
