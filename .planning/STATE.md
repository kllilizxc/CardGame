...
- `GameWorldStateWrite` is the aggregate explicit-storage plan/apply facade over the accepted Story/Hub, persistent-stash, and active-run write seams. It keeps deterministic slice ordering (`storyHubSession` → `persistentStash` → `activeRun`), clones compatibility/run-resolution metadata, supports direct document writes including nullable persistent-stash removal and route-scoped active-run save/clear, and deliberately remains separate from scene wiring plus `SaveWorldStateDocumentRestoreExecutor` / transfer flows.
