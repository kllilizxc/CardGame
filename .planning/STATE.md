...
- `GameWorldStateActiveRunWrite` is the narrow plan/apply seam over the current route-keyed `cardgame.active-run.v1:*` active-run compatibility boundary. It validates active-run compatibility metadata from `SAVE_COMPATIBILITY_REGISTRY`, requires explicit storage adapters, normalizes route identity through `RunPersistence` helpers, rejects document/route identity mismatches, deep-clones run documents and plans/results, and delegates save/clear apply steps (including legacy unscoped or legacy route-key cleanup) to `saveActiveRun` / `clearActiveRun`. `ExpeditionState` active-run save/reset and `RunResolution` battle-victory save / terminal clear now go through this writer while preserving the existing `loadActiveRun` read path and default localStorage/memory fallback via an explicit resolved storage adapter.
