...
- **薄大地图路由层**：`WorldMapScene` 读取 `public/data/world/world-map.json`，由大地图目的地持有 Hub / Hub location / Expedition 的目标身份与数据文件（如 `hubFile`、可选 `targetLocationId`、`mapFile`、事件/商店文件），再把主菜单统一路由到青云镇 Hub、青云宗山门 Hub、集市茶棚 Hub 地点或青云外山试炼 Expedition；Hub / Expedition 可用最小返回按钮回到大地图，并依赖各自本地 session / active run 恢复进度。
...
4. 玩家点击 `expedition` 目的地：`WorldMapScene` 创建 `sceneKey: "ExpeditionScene"` 的启动 intent，并把目的地声明的 `expeditionId`、`mapId`、world-state / starter-deck / map / event / shop 文件传给 Expedition；秘境准备、地图遍历、战斗往返和结算循环保持不变，战斗往返会保留这份 target config。
