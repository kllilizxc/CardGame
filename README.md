...
- **UnitEffectManager**：解析 `gongfa-list.json`，根据事件（如 `TurnEnd`）与条件(`ArtifactUsedThisTurn`)分派功法动作；`RecoverCardFromDiscard` / `SearchCardFromDeck` / `DrawAndFilter` 这类卡牌流转由 `gongfaCardOperations` 统一适配运行时副作用，`GainArmor` 由 `gongfaArmorOperations` 适配当前护甲日志与状态副作用，卡牌筛选与数值表达式仍复用纯 helper `gongfaCardFilter` / `gongfaExpression`。
...
4. **执行动作**：如 `RecoverCardFromDiscard`，`UnitEffectManager` 委托 `gongfaCardOperations`，用 `gongfaCardFilter` 匹配 `CardFilter`（含 `maxStar` 表达式）后从弃牌堆取出目标卡牌（“引剑者”效果）；`GainArmor` 仍由 `UnitEffectManager` 触发，但委托 `gongfaArmorOperations` 应用现有 `armor` 状态与日志副作用。
