...
- **数据驱动内容**：卡牌、功法、遭遇均来自 `public/data` JSON，可无代码迭代内容；`public/data/content-catalog.json` 作为只读清单覆盖当前 checked-in 内容，`src/game/content/contentCatalog.ts` 在测试中校验路径、JSON、领域 ID、首层 route-critical 引用与 Hub `startStory.storyResourceId` / `storyGraphFile` 对齐，但不改变运行时场景现有加载路径。
...
| `src/game/content/contentCatalog.ts` | 只读内容清单验证层：读取调用方提供的 catalog / public-file source，复用 WorldMap、Hub、Story 与 Expedition 纯校验器，并检查 checked-in 内容路径、领域 ID、首层 Hub / Story / Expedition 引用以及 Hub `startStory.storyResourceId` 是否解析到 kind 为 `story` 且 `publicPath` 等于 `storyGraphFile` 的 catalog 资源；运行时场景仍按各自现有 `data/...` 路径加载。 |
