...
| `public/data/content-catalog.json` | 版本化内容清单：列出当前 WorldMap、Hub、Story、Expedition、卡组、遭遇、卡牌、状态、功法、配置和世界种子 JSON；测试用它做纯验证，运行时由 `Preloader` 先加载，并由 `WorldMapScene` / `HubScene` / `StoryScene` / `ExpeditionScene` 等 runtime scene 用稳定资源 ID 解析入口与目标资源路径。 |
