...
- `src/game/state/GameWorldStatePersistentStashWrite.ts` 是 `GameWorldState` 之上的窄写入 seam：调用方必须提供显式 `Storage`-like adapter，它会校验当前 persistent-stash compatibility 仍指向 `cardgame.persistent-stash.v1`，先为 stored-stash 或 seed-fallback document 生成深拷贝 write plan，再在 apply 步骤交给 `savePersistentStash` 写回同一个 key。该层只物化永久仓库，不触碰 ambient `globalThis.localStorage`、Story/Hub session、route-keyed active run、场景 UI、结算经济或 content 数据。
