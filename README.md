...
| `src/game/content/contentIdRegistry.ts` | catalog-backed 内容 ID registry 与静态引用校验编排：登记卡牌、卡组、功法和世界物品 ID，并在运行前检查 deck/card/encounter/reward/gongfa 以及 canonical 初始世界 starter stash 的 deckRef、item id / itemType、count、spiritStones 引用与基础数值形状；canonical status.definitions 与 canonical config / realm / grade 校验分别委托给专用模块。 |
