...
- **UnitEffectManager**：解析 `gongfa-list.json`，根据事件（如 `TurnEnd`）与条件(`ArtifactUsedThisTurn`)执行动作（如恢复弃牌卡）；卡牌筛选规则本身由纯 helper `gongfaCardFilter` 统一判断，Manager 只负责手牌/牌库/弃牌堆移动与日志等运行时副作用。
...
4. **执行动作**：如 `RecoverCardFromDiscard`，用 `gongfaCardFilter` 匹配 `CardFilter`（含 `maxStar` 表达式）后从弃牌堆取出目标卡牌（“引剑者”效果）。
