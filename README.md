...
- **薄大地图路由层**：`WorldMapScene` 读取 `public/data/world/world-map.json`，由大地图目的地持有 Hub / Expedition 的目标身份与数据文件（如 `hubFile`、`mapFile`、事件/商店文件），再把主菜单统一路由到青云镇 Hub 或青云外山试炼 Expedition；Hub / Expedition 可用最小返回按钮回到大地图，并依赖各自本地 session / active run 恢复进度。
...
### HubScene 生命周期
1. `init` / `preload`：规范化启动 payload；若是直接启动则回退到 `hub.qingyun-town` / `data/hub/town-shell.json`，若来自大地图则载入 payload 指定的 `hubFile`。
...
| `public/data/world/world-map.json` | 第一张薄大地图：声明青云镇 Hub 与青云外山试炼 Expedition 目的地，并持有各目的地的 target identity / content files，由 `WorldMapScene` 路由。 |
