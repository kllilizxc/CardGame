...
- **UnitEffectManager**：解析 `gongfa-list.json`，根据事件（如 `TurnEnd`）与条件(`ArtifactUsedThisTurn`)触发功法动作；动作执行通过 `gongfaOperationDispatch` 的 typed registry 分派：`RecoverCardFromDiscard` / `SearchCardFromDeck` / `DrawAndFilter` 由 `gongfaCardOperations` 统一适配运行时副作用，`GainArmor` 由 `gongfaArmorOperations` 适配当前护甲日志与状态副作用，`ImmediateAttack` 由 `gongfaAttackOperations` 适配现有攻击运行时。卡牌筛选与数值表达式仍复用纯 helper `gongfaCardFilter` / `gongfaExpression`，尚未实现的动作只保留 warning + unexecuted 语义。
