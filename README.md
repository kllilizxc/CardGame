...
- **数据驱动世界地图壳层**：`WorldMapScene` 读取 `public/data/world/world-map.json` 渲染青云地域路线入口，由数据声明启动现有 `HubScene` 与 `ExpeditionScene`。
...
| `src/game/scenes/worldmap/WorldMapScene.ts` | 数据驱动世界地图壳层：读取 `public/data/world/world-map.json`，按 route 数据启动 `HubScene` 或 `ExpeditionScene`。 |
...
### Phaser 游戏实例
`src/game/main.ts` 配置 Phaser（画布尺寸、渲染、场景）。标准启动链路为 `Boot -> Preloader -> MainMenu -> WorldMapScene`：`Boot` 只加载预加载器所需的最小背景资源，`Preloader` 加载菜单资源后进入 `MainMenu`。玩家在 `MainMenu` 点击“进入大地图”后进入 `WorldMapScene`；`WorldMapScene` 读取 `public/data/world/world-map.json`，由 route 数据启动现有 `HubScene` 或 `ExpeditionScene`。`HubScene` 读取 `public/data/hub/town-shell.json`，可先按 `navigate.targetLocationId` 在城镇小地点间切换并保存当前位置，再由不同 `startStory.storyGraphFile` action 启动或恢复 `StoryScene`，例如 `public/data/story/story-graph.json` 主线或 `public/data/story/qingyun-teahouse-rumors.json` 茶棚支线。`BattleScene` 会被 StoryScene 的 `battleLaunch` 往返复用，`ExpeditionScene` 则作为世界地图上的秘境路线入口继续复用既有 run 流程。

### WorldMapScene 生命周期
1. `preload`：载入 `data/world/world-map.json`。
2. `create`：校验世界地图定义，渲染标题、说明和 route 节点，并发出 `EventBus.emit('current-scene-ready', this)`。
3. 玩家点击 route：`WorldMapScene` 通过 `createWorldMapRouteIntent` 从数据生成 scene launch intent；当前只允许启动现有 `HubScene` 与 `ExpeditionScene`，避免世界地图数据绕过既有壳层直接进入战斗或故事内部。
...
| `public/data/world/world-map.json` | 青云地域世界地图 route 壳层：声明“青云镇”启动 `HubScene` 与“青云外山试炼”启动 `ExpeditionScene`。 |
