...
- **剧情流视图模型**：`storyFlowViewModel` 将 `story-graph.json` 转换为 Phaser 可渲染的当前剧情、选项、推荐理由与缺失节点警告；`storyFlow` 仅负责 UI 示例图的严格结构 / 跳转校验。
...
| `src/game/scenes/story/StoryScene.ts` | 示例主线故事场景：读取 `story-graph.json`，渲染剧情节点、元数据、选项和终点重开按钮。 |
...
3. 玩家点击选项：`storyFlowViewModel` 根据当前节点和 choice id 计算下一节点视图；如果抵达无可见选项的节点，场景显示终点提示和“重新开始故事”按钮。`storyFlow` 在加载时只负责拒绝缺失节点、坏跳转等不可游玩的示例图。
...
| `public/data/story/story-graph.json` | 剧情图：节点、选项、条件提示与效果提示，由 `storyFlowViewModel` 转换为 UI 视图。 |
...
`src/game/scenes/story/storyFlowViewModel.ts` 是剧情 UI 的纯函数层，也是当前故事视图模型的唯一拥有者：输入剧情图与运行时剧情状态，输出当前节点文案、可见选项、推荐状态、选择跳转结果和内容配置警告。当前默认规则会根据 `player.attributes` 解析类似“心性 ≥ 50”的推荐条件；如果选项指向尚未配置的目标节点，视图模型会保留该选项但标记为不可选择，并提供中文警告，避免 StoryScene 因内容缺口崩溃。`src/game/scenes/story/storyFlow.ts` 保留为加载 `story-graph.json` 时的严格校验层，确保 checked-in 示例图本身可游玩。
