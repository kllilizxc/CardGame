...
│     ├─content/           # 只读内容目录 manifest 与纯校验 harness
...
| `src/game/content/contentCatalog.ts` | 只读内容目录校验 harness：解析 `public/data/content-catalog.json`，复用 WorldMap / Hub / Story / Expedition 纯 validator，并检查 route-critical JSON 引用是否都在目录中登记；运行时场景仍按原有 `load.json` 路径加载内容。 |
...
| 文件 | 说明 |
| --- | --- |
| `public/data/content-catalog.json` | 版本化只读内容目录 manifest，登记当前 checked-in route-critical JSON（大地图、Hub、Story、秘境、卡组、遭遇、卡牌与世界 seed），供 dev/test 阶段做纯校验；不参与运行时 scene loading。 |
...
内容迭代流程：修改 JSON → 同步更新 `public/data/content-catalog.json`（新增 / 移动 route-critical 文件时）→ `bun test src/game/content/contentCatalog.test.ts` 校验目录与首层路线引用 → `npm run dev` 热重载 → 通过 Hub 入口、剧情视图、战斗日志或调试功能验证。
