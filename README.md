...
- **UnitEffectManager**：解析 `gongfa-list.json`，根据事件（如 `TurnEnd`）与条件(`ArtifactUsedThisTurn`)分派功法动作；`RecoverCardFromDiscard` / `SearchCardFromDeck` / `DrawAndFilter` 这类卡牌流转由 `gongfaCardOperations` 统一适配运行时副作用，卡牌筛选仍复用纯 helper `gongfaCardFilter` / `gongfaExpression`。
...
4. **执行动作**：如 `RecoverCardFromDiscard`，`UnitEffectManager` 委托 `gongfaCardOperations`，用 `gongfaCardFilter` 匹配 `CardFilter`（含 `maxStar` 表达式）后从弃牌堆取出目标卡牌（“引剑者”效果）。
