...
- **剧情流视图模型**：`storyFlowViewModel` 将 `story-graph.json` 转换为 Phaser 可渲染的当前剧情、选项、推荐理由与缺失节点警告。
...
3. 玩家点击选项：`storyFlow` 根据当前节点和 choice id 计算下一节点视图；如果抵达无后续选项的节点，场景显示终点提示和“重新开始故事”按钮。
...
`src/game/scenes/story/storyFlowViewModel.ts` 是剧情 UI 的纯函数层：输入剧情图与运行时剧情状态，输出当前节点文案、可见选项、推荐状态、选择跳转结果和内容配置警告。当前默认规则会根据 `player.attributes` 解析类似“心性 ≥ 50”的推荐条件；如果选项指向尚未配置的目标节点，视图模型会保留该选项但标记为不可选择，并提供中文警告，避免 StoryScene 因内容缺口崩溃。
